		...
		$(function () {
				...
				form.ajaxSubmit(function (data) {
					if (data.success) {
						$.messager.confirm("提示", "审核成功!", function () {
							window.location.href = "/myVideoAuth";
						});
					} else {
			...
			$("#loginInfoDisplay").typeahead({
				source: function (query, process) {
					$.post("/autoComplete", {keyword: query}, function (data) {
						process(data);
					}, "json");
				},
				items: 5,
				...
				$("#loginInfoValue").val($(this).typeahead("getActive").id);
			});
		});
	</script>
</head>
...
<#include "../common/top.ftl"/>
	<div class="row">
		<div class="col-sm-3"><#assign currentMenu="/myVideoAuth" />
					...
					<div class="form-group">
						...
						<div class="col-sm-6">
							<div class="dropdown" id="autocomplate">
								...
								<input type="hidden" name="applierId" id="loginInfoValue"/>
							</div>
						</div>
					</div>
					<div class="form-group">
